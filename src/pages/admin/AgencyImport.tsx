  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < records.length; i += batchSize) {
        ...
        const transformedBatch = batch.map((record, index) => {
          try {
            return {
            ...
            console.error(`Transform error for record ${i + index}:`, err, record);
            errors.push({
              row: i + index + 2, // +2 for header and 0-index
              agency: record.agency_name || 'Unknown',
        ...
        if (error) {
          ...
          for (let j = 0; j < batch.length; j++) {
            ...
            const transformed = transformedBatch[j];
            if (transformed) {
      ...
      if (errors.length > 0) {
        ...
        const errorSummary = errors.slice(0, 10).map(e => 
          `Row ${e.row}: ${e.agency} - ${e.error}`
        ).join('\n');
        toast.error(`Import completed with ${errors.length} errors. First 10:\n${errorSummary}`);
      }
